...
using DemandRepaintCallback = void(*)();
...
private:
	...
	DemandRepaintCallback m_demandRepaint = nullptr;
...
public:
    ...
    void SetDemandRepaintCallback(DemandRepaintCallback callback)
    {
        m_demandRepaint = callback;
	}
    void RequestRepaint()
    {
...
void UpdateAIturn()
    ...
    case T_ATTACK:
    {
        ...
        if (m_config.m_Difficulty > 2 || player.success)
        {
...
void UpdateAttack()
{
    ...
    else
    {
        ...
        if (player.sant <= 0)
        {
            ...
            wchar_t data[256];
            swprintf_s(data, 256, L"Attack: Player %d FAIL, Cells: %d\n",
                m_activePlayer,
                m_field.CountPlayerCells(m_activePlayer));
    ...
    if (m_successfulCount >= m_config.m_AttackPerTurn)
    {
        ...
        wchar_t data[256];
        swprintf_s(data, 256, L"Attack: Player %d SUCCESS, Cells: %d\n",
            m_activePlayer,
            m_players[m_activePlayer].m_cells_owned);
        OutputDebugStringW(data);
...
void HandleAttackClick(ClickType click, int row = -1, int col = -1)
{
    if (click == CLICK_FIELD)
    {
        ...
        if (click == CLICK_CONFIRM)
        {
            ...
            wchar_t data[256];
            swprintf_s(data, 256, L"Human Attack: %d,%d -> %d,%d with %d ants\n",
                player.srow, player.scol, player.trow, player.tcol, player.sant);
            OutputDebugStringW(data);
...
void OnChar(char ch)
 ...
 void Render()
    {
        m_renderer.Render(m_field, m_players, m_dialog, GetPlayer(),
            m_antBox, m_error, m_gameSpeed);
    }
};
