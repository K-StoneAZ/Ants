...
public:
	...
	void setRowOffset(Field& game) {
		row_offset = game.getRows() * 3 + 1;
	}
	//printers
	...
	void Status(Field& game) {//player status area left edge 4
		setRowOffset(game);
		int towin = (game.getRows() * game.getCols() * 2) / 3;
		int row = getRowOffset();//calculate top line for status area
		game.SetColor(15, 1);
		for (int i = 0; i < 8; i++) {
			int a = i + row; game.Gotoxy(4, a);
		...
		game.Gotoxy(4, row); cout << "    Win STATS "<<towin;
		game.SetColor(7, 0);
	}
	void ToStatus(int player, int num, Field& game) {//left edge 4, top line 37
		int row = getRowOffset();//calculate top line for status area
		int a = row+1 + player; game.SetColor(15, 1);
		game.Gotoxy(20, a);
		printf("%d", num);
		game.SetColor(7, 0);
	}
...
void Dialog(Field& game) {//left edge 48, top line 37
	...
	game.SetColor(1, 7);
	for (int i = 0; i < 8; i++) {
		int a = i + row; game.Gotoxy(48, a);
		cout << "                                   ";// 35 character width x 9 lines
	}
	game.Gotoxy(61, row); cout << "Dialog";
	game.SetColor(7, 0);
}

//add this function to player class
void ToDialog(int line, string msg, Field& game) {//left edge 48, top line 37 max string len 33 char
	int row = getRowOffset();//calculate top line for status area	if (line == 0) { Dialog(game); }
	if (line == 0) { Dialog(game); }
	int line_offset = row + 1 + line;
	game.SetColor(1, 7);
	...
	bool Win(Field& game) const {
		...
		int counter = 0;
		for (int i = 1; i < numPlayers; i++) {
			if (playerStats[i].cells_owned > 0) {
				counter++;
			}
		}
		if (counter != 1) {
	...
	int runAttack(int playerNum, Field& game) {
		ToDialog(1,"Running Attack sequence:     ", game);
		Attack(game);
		...
		int bonus{};
		if (defend > 0) { bonus = (attack - defend) * 3; }
		ToAttack(playerNum, attack, defend, atroll, defroll, game);
		game.renderCell(srow, scol, true);
		game.renderCell(trow, tcol, true);
		if (game.getOwner(trow, tcol) == 0) {//step into unowned cell
			...
			while (defend != 0 && attack != 0) {
				atroll = (1 + rand() % 99) + bonus;
				if (atroll > 99) { atroll = 99; }
				if (atroll <= 0) { atroll = 0; }
				defroll = (1 + rand() % 99);
